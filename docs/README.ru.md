...
- Используя переменную `pageUrl`, вы можете выдать ссылке на текущую страницу, находящейся в главном меню (в хедере), активный класс ([подробнее](#)).
...
Чтобы узнать, как проще создавать компоненты, см. [Component generator]().
